...
def atomic_kernel(x_ptr,
    ...
    offsets2d = offsets[None, :]  + (tl.arange(0, 2) * BLOCK_SIZE)[:, None]
    x_ptrs = x_ptr + offsets2d
    v = tl.zeros([2, BLOCK_SIZE], dtype=tl.float16) + 1
...
def test_correctness(num_tiles):
    tile_size = 256
    x = torch.zeros((2, tile_size), dtype=torch.half, device='cuda')
    val = torch.ones((2, tile_size), dtype=torch.half, device='cuda')
    ...
    ref = torch.zeros((2, tile_size), dtype = torch.float, device='cuda') + num_tiles
